import { AlertTriangle, ShieldCheck, X, Clock, Video, BookOpen, AlertCircle, HelpCircle, Lock, Check } from "lucide-react";
      ...
      <div className="bg-white border border-slate-200 text-slate-800 shadow-xl rounded w-full max-w-4xl h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6 bg-slate-50">
          ...
          {attempt.lockState === "locked_awaiting_teacher" && (
            <div className="bg-amber-50 border border-amber-200 px-4 py-2.5 rounded text-xs text-amber-800 font-semibold flex items-center gap-2">
              <Lock className="w-3.5 h-3.5 shrink-0" />
              Session is currently locked — awaiting teacher approval to resume.
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white border border-slate-200 p-4 rounded shadow-sm">
              <span className="text-[9px] uppercase font-mono tracking-widest text-slate-400 font-bold block mb-1">Active Time</span>
            ...
            <div className="bg-white border border-slate-200 p-4 rounded shadow-sm">
              <span className="text-[9px] uppercase font-mono tracking-widest text-slate-400 font-bold block mb-1">Block Progress</span>
          ...
          {/* Section 1b: Per-block progress breakdown */}
          <div className="bg-white border border-slate-200 p-5 rounded shadow-sm">
            <h4 className="text-xs font-bold text-slate-800 border-b border-slate-100 pb-2 mb-3 uppercase tracking-wide">
              Lesson Progress by Block
            </h4>
            <div className="space-y-2">
              {lessonBlocks.sort((a: any, b: any) => a.order - b.order).map((block: any, idx: number) => {
                const isReached = idx <= attempt.currentBlockIndex;
                const isCompleted = idx < attempt.currentBlockIndex || attempt.status === "completed";
                const blockTimeSeconds = attempt.blockTimeSpent?.[block.id] || 0;
                const furthestSeconds = attempt.furthestVideoTimestamps?.[block.id];

                return (
                  <div key={block.id} className={`flex items-center gap-3 p-2.5 rounded text-xs ${isReached ? "bg-slate-50" : "opacity-40"}`}>
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${isCompleted ? "bg-green-100" : isReached ? "bg-blue-100" : "bg-slate-100"}`}>
                      {isCompleted
                    ...
                    <div className="flex items-center gap-1.5 min-w-0 flex-1">
                      {block.type === "video" ? <Video className="w-3 h-3 text-slate-400 shrink-0" /> : block.type === "question" ? <HelpCircle className="w-3 h-3 text-slate-400 shrink-0" /> : <BookOpen className="w-3 h-3 text-slate-400 shrink-0" />}
                      <span className="font-semibold text-slate-700 truncate">{block.title}</span>
                    </div>
                    <div className="flex items-center gap-3 shrink-0 text-[10px] font-mono text-slate-400">
                      {blockTimeSeconds > 0 && (
                        <span className="flex items-center gap-0.5">
                          <Clock className="w-2.5 h-2.5" />
                          {Math.floor(blockTimeSeconds / 60)}m {blockTimeSeconds % 60}s
                        </span>
                      )}
                      {block.type === "video" && furthestSeconds !== undefined && (
                        <span className="text-slate-500">{formatVideoTime(furthestSeconds)} watched</span>
                      )}
                    </div>
                  </div>
                );
