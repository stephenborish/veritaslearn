...
function SaveStatus({ state }: { state: AutosaveState }) {
  ...
  const config: Record<Exclude<AutosaveState, "idle">, { text: string; cls: string; spin?: boolean; alert?: boolean }> = {
    ...
    error: { text: "Couldn’t save — check your connection", cls: "text-rose-600", alert: true },
  };
  ...
  return (
    <div className="space-y-2.5">
      ...
      <div className="flex items-center justify-between gap-3 min-h-[18px]">
        ...
        <span className="text-xs text-slate-400 tabular-nums">{(value || "").length} characters</span>
      </div>
    </div>
  );
}
